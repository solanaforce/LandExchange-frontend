import { Currency, CurrencyAmount, Native, Token } from 'libraries/swap-sdk'
...
export function useNativeBalances(uncheckedAddresses?: (string | undefined)[]): {
  ...
  const addresses: Address[] = useMemo(
    () =>
      uncheckedAddresses ? orderBy(uncheckedAddresses.map(safeGetAddress).filter((a): a is Address => a !== undefined)) : [],
    [uncheckedAddresses],
  )
