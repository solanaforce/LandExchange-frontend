...
export function useSingleContractMultipleData<TAbi extends Abi | readonly unknown[]>({
  contract,
  args,
  functionName,
  options,
}: // FIXME: wagmiv2
SingleContractMultipleDataCallParameters<TAbi>): CallState<any>[] {
  const { chainId } = useActiveChainId()
...
export function useMultipleContractSingleData<TAbi extends Abi | readonly unknown[]>({
  abi,
  addresses,
  functionName,
  args,
...
MultipleSameDataCallParameters<TAbi>): CallState<any>[] {
...
export function useSingleCallResult<TAbi extends Abi | readonly unknown[]>({
  contract,
  functionName,
  args,
  options,
}: // FIXME: wagmiv2
SingleCallParameters<TAbi>): CallState<any> {
