...
import { TokenInfo } from 'libraries/token-lists'
...
export function filterTokens(tokens: Token[], search: string): Token[] {
  ...
  const searchingAddress = safeGetAddress(search)
...
export function createFilterToken<T extends TokenInfo | Token>(search: string): (token: T) => boolean {
  ...
  return (token) => {
    const { symbol, name } = token
    return (symbol ? matchesSearch(symbol) : false) || (name ? matchesSearch(name) : false)
  }
}
