...
const Presale = ({countdownNode}: {countdownNode: React.ReactNode}) => {
  const { totalInvestors, totalDeposit, paused, finalized } = usePublicInfo()
  ...
  const isDepositDisabled = new BigNumber(val).times(10 ** 18).isGreaterThan(btfBalance?.toString() ?? "0") ||
    ...
    new BigNumber(val).times(10 ** 18).plus(deposit?.toString() ?? "0").isLessThan(1e17)
  ...
  return (
    <Page>
      <Flex width="100%" flexDirection={["column", "column", "column", "column", "column", "column", "column"]} justifyContent="center">
        <Box
          ...
          <StyledAppBody>
            <Box p="20px">
              ...
              <Box mt="15px">
                ...
                <Text textAlign="center" fontSize="36px">Presale Information</Text>
                <Box mt="20px">
        ...
        >
          <StyledAppBody>
            <Box p="20px">
              ...
              {finalized && <Flex justifyContent="space-between" alignItems="center" mt="20px">
                <Box />
                <Button
                  onClick={() => onClaim()}
